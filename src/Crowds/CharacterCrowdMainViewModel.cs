...
namespace HeroVTT.Crowds
{
    public class CharacterCrowdMainViewModel : BaseViewModel
    {
            ...
            : base(busyService, container)
        {
            ...
            this.eventAggregator.GetEvent<AddToRosterEvent>().Subscribe(_ =>{ this.IsRosterExplorerExpanded = true; });
            this.eventAggregator.GetEvent<EditCharacterEvent>().Subscribe(_ => { this.IsCharacterEditorExpanded = true; });
            this.eventAggregator.GetEvent<EditIdentityEvent>().Subscribe(_ => { this.IsIdentityEditorExpanded = true; });
            this.eventAggregator.GetEvent<EditAbilityEvent>().Subscribe(_ => { this.IsAbilityEditorExpanded = true; });
            this.eventAggregator.GetEvent<EditMovementEvent>().Subscribe(_ => { this.IsMovementEditorExpanded = true; });
            this.eventAggregator.GetEvent<CreateCrowdFromModelsEvent>().Subscribe(_ => { this.IsCrowdFromModelsExpanded = true; });
