...
| Region | Type | Slot | Controls | Interaction decisions |
| --- | --- | --- | --- | --- |
| toolbar | toolbar | panel | New, Cut, Find, Clone, Paste, +Crowd, Del, Edit (8 icon buttons in horizontal row) | Matches production toolbar icons; each button activates corresponding action on selected tree node |
| filter bar | form | panel | Filter by Name (text input) | Live substring match; collapses non-matching nodes; filter text shown in field |
| crowd tree | tree | panel | Hierarchical crowd/character tree with expand/collapse (`+`/`-` prefixes) | `+` = collapsed crowd, `-` = expanded crowd; character leaves indented under parent crowd; selected item highlighted blue; supports drag-drop, inline rename, right-click context menu |
| collapsed side tabs | collapsed-tabs | side | Active Roster tab, Edit Character tab | Vertical text with `+` icon; click expands the panel and collapses others |

### Controls detail — crowd tree

| Control | Type | State / behavior |
| --- | --- | --- |
| crowd node (`+`/`-`) | tree node | Click `+` to expand, `-` to collapse; bold when expanded; shows child count |
...
| Save | Ctrl+S | Saves crowd collection to repository |
...
| Save Crowd Collection | toolbar / context menu | Save button or Ctrl+S |
...
| --- | --- | --- |
| structure commands | menu-group | New, Edit, Delete, Save |
