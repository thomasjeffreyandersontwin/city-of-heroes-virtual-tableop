...
(E) Manage Characters and Crowds
    (E) Manage Crowd Repository
        ...
        (S) System --> Load Crowd Collection from Repository
    ...
    (E) Organize Crowd Collections
        (S) GM --> Browse Crowds by Concept (Animals, Armed Forces, Civilians, Vehicles, etc.)
        (S) GM --> Browse Crowds by Gangs, Crews, and Squads
        (S) GM --> Browse Crowds by COH Structure
        (S) GM --> Browse All Characters Crowd
...
(E) Control Character Movement
    ...
    (E) Special Movement Modes
        ...
        * Gap: Movement types beyond defaults (Fly, Jump, Steam Jump Jetpack, Beast, Ninja Jump, Ice Slide) — partially implemented as activation key mappings; unclear which are fully functional vs. cosmetic.
...
### Default Abilities (Recovery, Stun Recovery, Pass Turn, …, Generic Damage/Power)
Groups 19 default abilities into one "Add Default Abilities" story. All share the same pattern: load from global default list and attach to character.
AC must specify per variant:
- Combat maneuvers (Strike, Haymaker, Sweep, Move By, Move Through, Grab, Disarm): each has different in-game FX and animation sequences
- Defensive maneuvers (Dodge, Block, Set): different activation contexts and visual indicators
- Status actions (Recovery, Stun Recovery, Prone, Off Ground): different state transitions
- Turn management (Pass Turn, Half Phase Action, Hold Action): affect turn order flow
- Utility (Draw A Weapon, Rapid Fire, Generic Damage/Power): different animation element compositions

### Movement Types (Walk, Run, Swim, Fly, Jump, Steam Jump Jetpack, Beast, Ninja Jump, Ice Slide)
Groups 9 movement types that share the same Move/Turn/Pause/Resume interface.
AC must specify per variant:
- Walk/Run/Swim: default movements added to every character; differ in speed and animation
- Fly: vertical movement allowed, no collision detection with floor
- Jump/Steam Jump Jetpack: arc-based movement with gravity
- Beast/Ninja Jump/Ice Slide: specialized locomotion animations with different visual effects

### Attack Effects (Stunned, Unconscious, Dying, Dead)
Groups 4 status effects that share the same "apply to defender" pattern.
AC must specify per variant:
- Stunned: temporary incapacitation, recoverable via Stun Recovery ability
- Unconscious: extended incapacitation, recoverable via Recovery ability
- Dying: critical state, may recover or progress to Dead
- Dead: terminal state, character remains on desktop but is inactive

### Identity Types (Model, Costume)
Groups 2 identity types with different rendering workflows.
AC must specify per variant:
- Model: uses COH NPC model name; supports ghost shadow overlay; spawns with model directly
- Costume: uses .costume file from COH costumes directory; supports original/persistent/ghost variants; loaded via LoadCostume command after spawn

## Context Gaps

- No player-facing interface exists yet — only the GM interacts with the application. Unclear whether multi-player or player self-service is planned.
- Map loading capability exists as a GameEvent (LoadMap) but no UI or workflow for map selection is visible in the current codebase.
- The pop-up menu system (COH menus) is referenced in design docs as needing dynamic reload capability — current implementation writes files but may require game restart to take effect.
- Scene concept appears in the StarUML domain model (Name, AwayPoints, Load) but no Scene management UI or workflow is implemented in the current WPF codebase — unclear whether scene staging is planned beyond individual character positioning.
- The external HCS (Hero Combat System) application that writes EventInfo files is not part of this repository — its behavior is inferred from the file-watcher integration; the full HCS event protocol (file naming, payload schema, timing) is not documented in the available sources.
